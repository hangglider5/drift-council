...
- **Frontend**: Phaser, Vite, plain TypeScript and DOM APIs
- **Backend**: Node.js v22 serverless environment (Devvit), Hono
- **Communication**: Typed JSON over Hono REST routes

## Layout & Architecture

- `/src/server`: **Backend Code**. This runs in a secure, serverless environment.
  - `index.ts`: Main server entry point (Hono app).
  - `routes/api.ts`: Defines the client-facing Hono REST API routes.
  - `routes/menu.ts`: Handles the moderator menu action registered in `devvit.json`.
  - `core/post.ts`: Creates the app's custom Reddit post.
...
- `/src/client`: **Frontend Code**. This is executed inside of an iFrame on reddit.com
  ...
  - Entrypoints:
    - `game.html` / `game.ts`: The Phaser game entry point (Expanded View).
    - `splash.html` / `splash.ts`: The plain TypeScript and DOM entry point (Inline View). This will be shown in the reddit.com feed. Please keep it fast and keep heavy dependencies inside of `game.html`.
